import { attachListener } from "./servicesEvents.mjs";
...
function rotateArray(array, degrees) {
	...
	return degrees > 0
		? [
				...array.slice(array.length - itemsToSlice),
				...array.slice(0, array.length - itemsToSlice),
			]
...
function attachPan(_canvas) {
	...
	if (serviceMapTransform) {
		...
		_canvas.style.transform = modifyTransform(
			_canvas,
			"translate",
			`${transX}px,${transY}px`
		);
...
function circleLayout({
	...
	for (var i = 0; i < numberOfItems; i++) {
		...
		var y1 = Math.round(
			height / 2 + verticalScale * radius * Math.sin(angle) - itemHeight / 2
		);
	...
	for (var i = 0; i < positions.length; i++) {
		if (i <= positions.length / 2) {
			const previous = i === first ? positions[last] : positions[i - 1];
		...
		const next = i === last ? positions[first] : positions[i + 1];
...
function addLinks({ canvas, allNodes }) {
	const createLine = ({ source, target }) => {
		...
		line.adjust = ({ parent, diff }) => {
			if (![source.parent, target.parent].includes(parent)) return;
	...
	for (let t = 0; t < triggers.length; t++) {
		...
		const connections = targets.filter(
			(target) => target.label === trigger.label
		);
		const theseLines = connections.map((conn) =>
			createLine({
				target: conn,
				source: trigger,
				x: trigger.x1,
...
function drawRootNodes({ canvas, listeners, triggers, sidebar }) {
	const keyLength = (k) =>
		(listeners[k] || []).length + (triggers[k] || []).length;
	...
	for (var i = 0, len = keys.length; i < len; i++) {
		...
		const children = [
			...(listeners[keys[i]] || []),
			...(triggers[keys[i]] || []).map((key) => ({ key, type: "trigger" })),
		];
		const nodeParams = {
			x: x / 2 - 20,
			y: y / 2 + 5,
		...
		node.addEventListener("synthetic-drag", ({ detail }) => {
			myLinks =
				myLinks ||
				allLinks.filter(
					(x) => x.source.parent === nodeName || x.target.parent === nodeName
				);
			myLinks.forEach((m) =>
				m.line.adjust({
					parent: nodeName,
			...
			const serviceMapPositionsUI = allParentNodes.map((x) =>
		...
		node.onclick = (event) => {
			if (node.classList.contains("selected")) {
				...
				Array.from(
					document.querySelectorAll("#canvas .node.selected")
	...
	const serviceMapPositionsUI = (
		localStorage.getItem("serviceMapPositionsUI") || ""
	).split(",");
	...
	const selectInstance = M.FormSelect.init(select, {
		...
		dropdownOptions: {
			...
			onCloseEnd: (e) => {
				...
				const selected =
					parent &&
					document.querySelector(".selector ul.select-dropdown li.selected");
				const items =
					selected &&
					Array.from(
...
function getZoomControls(el, zoomTarget) {
	...
	el.querySelector(".zoom-plus").onclick = (e) => {
		...
		zoomTarget.style.transform = modifyTransform(
			zoomTarget,
			"scale",
			Number(zoom) + 0.1
		);
...
const handleSelect = (
	...
	const cleanCanvas = () => {
		const allGroups = Array.from(canvas.querySelectorAll("g")).forEach((g) => {
