(() => {
  ...
  const fakeExpress = ({ storage, TemplateEngine }) => {
    const {
      files: filesStore,
    ...
    const addServiceHandler = async function ({ name, msg }) {
      const route = `^/${name}/(.*)`;
      const handler = "./modules/serviceRequestHandler.js"; //TODO: this will change!
      ...
      if (foundExactHandler) {
        ...
        await this.handlersStore.setItem(route, {
          type,
          route,
          handlerName: handler,
          handlerText: foundHandler.handlerText,
    ...
    const restorePrevious = async ({ servicesStore }) => {
    ...
    const find = async (url) => {
      ...
      if (!found) {
        await restorePrevious({ servicesStore });
    ...
    const app = {
      addServiceHandler: addServiceHandler.bind({ handlersStore }),
      get: generic("get"),
      post: generic("post"),
      find,
    };
