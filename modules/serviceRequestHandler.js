...
const fakeExpress = ({ store, handlerStore, metaStore }) => {
	...
	const addServiceHandler = async ({ name, msg }) => {
		...
		if (foundExactHandler) {
			console.log(
				`sw handler was already installed for ${foundExactHandler.routePattern} (boot)`
			);
		} else {
			handlers.push({
...
async function getCodeFromStorageUsingTree(tree, store, serviceName) {
	...
	for (let index = 0; index < files.length; index++) {
		...
		if (file.path.includes("/welcome/")) {
			storedFile =
				allFilesFromService["." + file.path.replace("/welcome/", "/.welcome/")];
		} else {
...
const UIManagerInit = async (manager, { handlerStore, changeStore }) => {
	...
	const foundExactHandler =
		foundHandler &&
		handlers.find((x) => x.handlerName === handler && x.routePattern === route);
...
class UIManager {
	...
	static init = (handlerStore, changeStore) =>
...
const providerFileChange = async ({
	path,
	code,
	parent,
	metaStore,
	...
	const pathWithoutParent = path.replace(
		"./" + (serviceName || foundParent.name),
		""
	);
...
(() => {
	...
	(async () => {
		mimeTypes = await fetchJSON(
			"https://cdn.jsdelivr.net/npm/mime-db@1.45.0/db.json"
		);
	})();
	...
	const providerStore = localforage.createInstance({
		driver: driverOrder,
		name: "serviceRequest",
		version: 1.0,
		storeName: "provider",
	...
	const changeStore = localforage.createInstance({
		driver: driverOrder,
		name: "serviceRequest",
		version: 1.0,
		storeName: "changes",
	...
	app.get("/service/read/:id?", async (params, event) => {
		...
		if (!params.id || params.id === "*") {
			...
			for (var i = 0, len = savedServices.length; i < len; i++) {
				...
				const code = await getCodeFromStorageUsingTree(
					service.tree,
					store,
					service.name
				);
		...
		if (foundService) {
			foundService.code = await getCodeFromStorageUsingTree(
				foundService.tree,
				store,
				foundService.name
			);
	...
	app.post("/service/update/:id?", async (params, event) => {
		try {
			...
			const parsedCode =
				!Array.isArray(body.code) && safe(() => JSON.parse(body.code));
			...
			const storageFiles = await getCodeFromStorageUsingTree(
				body.tree,
				store,
				service.name
			);
			const updateAsStore = getCodeAsStorage(
				body.tree,
				body.code,
				service.name
			);
	...
	app.post("/service/provider/test/:id?", async (params, event) => {
		try {
			...
			const isSupported = ["basic-bartok-provider", "github-provider"].includes(providerType);
			if (!isSupported) {
				return JSON.stringify(
					{
						error: `Unsupported provider type: ${providerType}`,
			...
			if (providerType === 'github-provider') {
				return JSON.stringify({ success: true, todo: "test user's access token" })
			}
			const fileUrl = (providerUrl + "/file/").replace("//file/", "/file/");
			const treeUrl = (providerUrl + "/tree/").replace("//tree/", "/tree/");
			try {
	...
	async function serviceAPIRequestHandler(event) {
		...
		try {
			const splitPath = event.request.url
				...
				.split("/");
			if (
				splitPath.includes("::preview::") &&
				splitPath.includes(UIManager.name)
			) {
