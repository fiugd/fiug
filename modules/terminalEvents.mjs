...
const viewSelectHandler = ({ viewUpdate }) => (event) => {
  ...
  let code = typeof currentFile === "string" ? currentFile : "";
...
const terminalActionHandler = ({ terminalActions, viewUpdate }) => (event) => {
  ...
  let code;
  code = typeof currentFile === "string" ? currentFile : "";

  const isHTML =
    code.includes("</html>") &&
    ["htm", "html"].find((x) => {
      return currentFileName.includes("." + x);
    });
  const isSVG =
    code.includes("</svg>") &&
    ["svg"].find((x) => {
    ...
    if ((!id && id !== 0) || id === NaN) {
      ...
      after = ({ result }) => {
        ...
        const services = (result.result || [])
          ...
          .map((x) => `${(x.id || 0).toString().padStart(5, " ")}   ${x.name}`)
          .join("\n");
        loading(
          `\n
				${services}
...
const handleCommandQueue = (event) => {
  ...
  const foundQueueItem =
    listener && queuedCommands.find((x) => x.id === listener);
  if (!foundQueueItem) {
    return false;
  }
...
const operationDone = ({ viewUpdate, viewReload }) => (event) => {
  ...
  const defaultFileContents = (
    result[0].code.find((x) => x.name === defaultFile) || {}
  ).code;
...
const operations = ({ viewUpdate, getCurrentService }) => (event) => {
  ...
  const lastChange = (() => {
    try {
      return Object.entries(state.changedFiles)
        ...
        .map(([key, value]) => value[value.length - 1])[0].code;
    } catch (e) {}
  })();
  ...
  const contents = (lastChange ? lastChange : getFileFromService(name)) || "";
